package com.chargerwallet.app.wallet.lite
...
import com.chargerwallet.app.wallet.lite.nfc.NFCExceptions
import com.chargerwallet.app.wallet.lite.nfc.NfcUtils
import com.chargerwallet.app.wallet.lite.nfc.broadcast.NfcStatusChangeBroadcastReceiver
import com.chargerwallet.app.wallet.lite.chargerwalletLite.ChargerWalletLiteCard
import com.chargerwallet.app.wallet.lite.chargerwalletLite.entitys.CardState
import com.chargerwallet.app.wallet.lite.utils.NfcPermissionUtils
import com.chargerwallet.app.wallet.lite.utils.Utils
...
//                            mNFCConnectedChannel.receive()
                            val startRequest = ChargerWalletLiteCard.initRequest(isoDep)
                            val dataMap = Arguments.createMap().apply {
                                putInt("code", -1)
                                putString("type", "ChargerWallet_Lite")
    ...
    override fun initialize() {
        ...
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                ChargerWalletLiteCard.startNfc(it as FragmentActivity) {}
            }
        }
    ...
    override fun onHostResume() {
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                ...
                try {
                    ChargerWalletLiteCard.startNfc(it) {
    ...
    override fun onHostPause() {
        Utils.getTopActivity()?.let {
            launch(Dispatchers.IO) {
                try {
                    ChargerWalletLiteCard.stopNfc(it as FragmentActivity)
    ...
    private suspend fun acquireDevice(): IsoDep? {
        ...
        if (receiveIsoDep == null) {
            ...
            val initChannelRequest = ChargerWalletLiteCard.initRequest(receiveIsoDep)
    ...
    fun getLiteInfo(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val cardInfo = ChargerWalletLiteCard.getCardInfo(isoDep)
    ...
    fun setMnemonic(mnemonic: String, pwd: String, overwrite: Boolean, callback: Callback) =
        launch {
            handleOperation(callback) { isoDep ->
                ...
                val isSuccess =
                    ChargerWalletLiteCard.setMnemonic(mCurrentCardState, isoDep, mnemonic, pwd, overwrite)
    ...
    fun getMnemonicWithPin(pwd: String, callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            ChargerWalletLiteCard.getMnemonicWithPin(mCurrentCardState, isoDep, pwd)
        }
    }
    ...
    fun changePin(oldPwd: String, newPwd: String, callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            ChargerWalletLiteCard.changPin(mCurrentCardState, isoDep, oldPwd, newPwd)
        }
    }
    ...
    fun reset(callback: Callback) = launch {
        ...
        handleOperation(callback) { isoDep ->
            ...
            val isSuccess = ChargerWalletLiteCard.reset(isoDep)
            if (!isSuccess) throw NFCExceptions.ExecFailureException()
